...
from app.models.join_request import JoinRequest, ModeratorInvite, DirectModeratorInvite, ExistingUserInvite
from app.forms.join_request_forms import AdminJoinRequestForm, DirectModeratorInviteForm
...
def edit_moderator_permissions(user_id):
    ...
    if form.validate_on_submit():
        try:
            # Use the new 2-tier permission system
    ...
    if form.validate_on_submit():
        try:
            ...
            for user_id in user_ids:
                ...
                # Note: data_range no longer exists in the new 2-tier system
                # if form.data_range_access.data:
                #     permissions.data_range = form.data_range_access.data
    ...
    if request.method == 'POST':
        ...
        if action == 'approve':
            ...
            try:
                ...
                existing_user = User.query.filter_by(email=join_request.email).first()
                
                if existing_user:
                    # Check if user is already in a company
                    if existing_user.company_id:
                        # Check if they're admin of their current company
                        if existing_user.company and existing_user.company.admin_id == existing_user.id:
                            flash('This user is an administrator of another company and cannot be approved.', 'error')
                            return render_template('company_admin/review_join_request.html', 
                                                 join_request=join_request, form=form)
                        
                        # Check if they're already in this company
                        if existing_user.company_id == join_request.company_id:
                            flash('This user is already a member of your company.', 'error')
                            return render_template('company_admin/review_join_request.html', 
                                                 join_request=join_request, form=form)
                        
                        # Leave current company first
                        existing_user.company_id = None
                        existing_user.role_company = None
                    
                    # Update existing user to join the company
                    existing_user.company_id = join_request.company_id
                    existing_user.role_company = 'moderator'
                    # If user was a subscriber, change to viewer when joining as team member
                    if existing_user.role_website == 'subscriber':
                        existing_user.role_website = 'viewer'
                    
                    # Update user information if provided in the join request
                    if join_request.first_name:
                        existing_user.first_name = join_request.first_name
                    if join_request.last_name:
                        existing_user.last_name = join_request.last_name
                    if join_request.username and not existing_user.username:
                        existing_user.username = join_request.username
                    
                    user = existing_user
                else:
                    # Create new user for email-only join requests
                    user = User(
                ...
                # Create or update permissions with the selected level
                permissions = UserPermissions.query.filter_by(
                    user_id=user.id,
                    company_id=join_request.company_id
                ).first()
                ...
                elif role_permissions == 'full_access':
                    permissions.set_permissions(
                ...
                if existing_user:
                    # Send welcome email to existing user (no password reset needed)
                    from app.utils.email import send_join_approved_email
                    try:
                        send_join_approved_email(join_request, user, role_permissions)
                    except:
...
def invite_moderator():
    """Send direct moderator invite"""
    company = current_user.company
    form = DirectModeratorInviteForm()
        ...
        try:
            ...
            if existing_user:
                ...
                if existing_user.company_id:
                    # Check if they're admin of their current company
                    if existing_user.company and existing_user.company.admin_id == existing_user.id:
                        flash('This user is an administrator of another company and cannot be invited.', 'error')
                        return render_template('company_admin/invite_moderator.html', form=form, company=company)
                ...
                # Create existing user invite
                invite = ExistingUserInvite(
                ...
                send_existing_user_invite_email(invite)
                
                flash(f'Invitation sent to existing user {existing_user.full_name} at {existing_user.email}!', 'success')
                return redirect(url_for('company_admin.direct_invites'))
            else:
                # Create direct moderator invite for new user
                ...
                flash(f'Invitation sent to {invite.full_name} at {invite.email}!', 'success')
                return redirect(url_for('company_admin.direct_invites'))
...
@company_admin_required
def direct_invites():
    """View sent direct invites"""
    page = request.args.get('page', 1, type=int)
    
    # Get both types of invites for the current user's company
    direct_invites = DirectModeratorInvite.query.filter_by(company_id=current_user.company_id)\
                                                .order_by(DirectModeratorInvite.created_at.desc())\
                                                .all()
    
    existing_user_invites = ExistingUserInvite.query.filter_by(company_id=current_user.company_id)\
                                                    .order_by(ExistingUserInvite.created_at.desc())\
                                                    .all()
    ...
    for invite in direct_invites:
        all_invites.append({
            ...
            'status': 'used' if invite.is_used else ('expired' if invite.is_expired else 'pending'),
            'expires_at': invite.expires_at
        })
    
    # Add existing user invites
    for invite in existing_user_invites:
        all_invites.append({
            'type': 'existing_user',
            'invite': invite,
            'email': invite.email,
    ...
    paginated_invites = all_invites[start:end]
